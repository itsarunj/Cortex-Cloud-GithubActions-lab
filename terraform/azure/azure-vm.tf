# main.tf blah - Terraform code to create a cclabtest Azure VM for demonstration.

terraform {
  required_providers {
    azurerm = {
      source  = "hashicorp/azurerm"
      version = "~> 4.38.1"
    }
  }

  # This backend block configures Terraform to store its state file
  # in an Azure Storage Account. This is the fix for the "resource already exists" error.
  backend "azurerm" {
    resource_group_name  = "itsarunj-tfstate-rg"
    storage_account_name = "itsarunjtfstatestorage"
    container_name       = "itsarunj-tfstate"
    key                  = "itsarunj-terraform.tfstate"
  }
}

# This provider block configures the Azure provider.
# It will now automatically read the subscription ID from the environment.


provider "azurerm" {
  features {}
}

# -------------------------------------------------------------------------------------
# Intentionally Flawed Code for Demonstration Purposes
# -------------------------------------------------------------------------------------

# A new variable to track the resource owner via GitHub username.
# This value should be passed from the GitHub Actions workflow.
variable "github_username" {
  description = "The GitHub username of the person who triggered the deployment."
  type        = string
}

# Flaw 1: Hardcoding sensitive information (admin password).
# Hardcoding secrets directly in the code is a significant security risk.
# This value should be managed securely using a tool like GitHub Secrets or Azure Key Vault.
variable "admin_password" {
  description = "The admin password for the VM. This is a flaw and should not be hardcoded."
  type        = string
  sensitive   = true
}

# Flaw 2: Overly permissive Network Security Group (NSG) rule.
# This rule allows all inbound traffic from any source to any port.
# This is a major security flaw and exposes the VM to the public internet without restriction.
resource "azurerm_network_security_group" "cclabtest_nsg" {
  name                = "${var.github_username}-cclabtest-nsg"
  location            = "eastus"
  resource_group_name = azurerm_resource_group.cclabtest_rg.name

  security_rule {
    name                         = "allow_all_inbound"
    priority                     = 100
    direction                    = "Inbound"
    access                       = "Allow"
    protocol                     = "*"
    source_port_range            = "*"
    destination_port_range       = "*"
    source_address_prefix        = "*"
    destination_address_prefix   = "*"
  }
}

# Resource group for the VM.
resource "azurerm_resource_group" "cclabtest_rg" {
  name     = "${var.github_username}-cclabtest-rg"
  location = "eastus"
}

# Virtual network for the VM.
resource "azurerm_virtual_network" "cclabtest_vnet" {
  name                = "${var.github_username}-cclabtest-vnet"
  address_space       = ["10.0.0.0/16"]
  location            = azurerm_resource_group.cclabtest_rg.location
  resource_group_name = azurerm_resource_group.cclabtest_rg.name
}

# Subnet for the VM.
resource "azurerm_subnet" "cclabtest_subnet" {
  name                 = "${var.github_username}-cclabtest-subnet"
  resource_group_name  = azurerm_resource_group.cclabtest_rg.name
  virtual_network_name = azurerm_virtual_network.cclabtest_vnet.name
  address_prefixes     = ["10.0.2.0/24"]
}

# Public IP address for the VM.
resource "azurerm_public_ip" "cclabtest_public_ip" {
  name                = "${var.github_username}-cclabtest-public-ip"
  location            = azurerm_resource_group.cclabtest_rg.location
  resource_group_name = azurerm_resource_group.cclabtest_rg.name
  allocation_method   = "Dynamic"
  sku                 = "Basic"
}

# Network interface for the VM.
resource "azurerm_network_interface" "cclabtest_nic" {
  name                = "${var.github_username}-cclabtest-nic"
  location            = azurerm_resource_group.cclabtest_rg.location
  resource_group_name = azurerm_resource_group.cclabtest_rg.name

  ip_configuration {
    name                          = "internal"
    subnet_id                     = azurerm_subnet.cclabtest_subnet.id
    private_ip_address_allocation = "Dynamic"
    public_ip_address_id          = azurerm_public_ip.cclabtest_public_ip.id
  }
}

# New resource to associate the NSG with the NIC.
resource "azurerm_network_interface_security_group_association" "cclabtest_nsg_association" {
  network_interface_id        = azurerm_network_interface.cclabtest_nic.id
  network_security_group_id   = azurerm_network_security_group.cclabtest_nsg.id
}

# Flaw 3: Using a known insecure or outdated VM image.
resource "azurerm_linux_virtual_machine" "cclabtest_vm" {
  name                = "${var.github_username}-cclabtest-vm"
  resource_group_name = azurerm_resource_group.cclabtest_rg.name
  location            = azurerm_resource_group.cclabtest_rg.location
  size                = "Standard_DS1_v2"
  admin_username      = "vmadmin"
  admin_password      = var.admin_password
  disable_password_authentication = false

  network_interface_ids = [
    azurerm_network_interface.cclabtest_nic.id,
  ]

  os_disk {
    caching              = "ReadWrite"
    storage_account_type = "Standard_LRS"
  }

  source_image_reference {
    publisher = "Canonical"
    offer     = "UbuntuServer"
    sku       = "18.04-LTS" # Flaw: Using an old Ubuntu 18.04 LTS image. A scanner should recommend a newer, supported version like 22.04 LTS.
    version   = "latest"
  }
}

# -------------------------------------------------------------------------------------
# Outputs
# -------------------------------------------------------------------------------------

output "public_ip_address" {
  value = azurerm_public_ip.cclabtest_public_ip.ip_address
}
